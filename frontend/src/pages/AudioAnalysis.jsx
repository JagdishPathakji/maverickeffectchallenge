...
import { FileSearch, Loader2, UploadCloud, Mic } from 'lucide-react';
...
const AudioAnalysis = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/analysis/audio', formData, {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Audio Scanner</h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Upload an audio recording of a suspicious phone call or voice message. We will transcribe and analyze it.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              ...
              {!preview ? (
                ...
                <div className="relative rounded-lg overflow-hidden border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 p-4 text-center">
                  <audio ref={audioRef} src={preview} controls className="w-full mb-2" />
                  <p className="text-sm text-gray-600 dark:text-gray-400 truncate px-8">{file.name}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              ...
              <button type="submit" disabled={loading || !file} className="w-full sm:w-auto btn-primary flex items-center justify-center disabled:opacity-50">
                {loading ? <><Loader2 className="animate-spin -ml-1 mr-2 h-5 w-5" /> Analyzing Audio...</> : 'Scan Audio'}
              </button>
            </div>
          </form>
